...
- `tests/test_data.py`
  - **Reproducibility**
    - Check hashes match for `.pt` data.
    - For spatial datasets (`.gpkg`), hashing is unreliable due to SQLite's internal state, so we compare their geometry and relevant fields (`coarse_id`, `X_common`, `group_effect`, `X_climate1–3`, `X_socio1–3`, `y1–3`).
